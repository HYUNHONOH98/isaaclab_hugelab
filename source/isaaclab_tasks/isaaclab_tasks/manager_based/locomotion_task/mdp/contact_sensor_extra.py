...
from isaaclab.sensors import ContactSensor, ContactSensorData
...
class ContactSensorExtra(ContactSensor):
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        if set_sparse:
            ...
            msk_ovf = cum_f >= c
            num_f[msk_ovf] = (c - cum_f - num_f).clamp_min_(0)[msk_ovf].to(dtype=num_f.dtype)
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            if not hasattr(self, "contact_visualizer_2"):
                self.contact_visualizer_2 = VisualizationMarkers(self.cfg.visualizer_cfg_2)
    ...
    def _debug_vis_callback(self, event):
        ...
        if torch.any(c_has):
            ...
            arrow_scale = th.tensor([1.0, 0.01, 0.01], device=self.device).repeat(quat_xyzw.shape[0], 1) * 6
            self.contact_visualizer_2.visualize(point.view(-1, 3), xyzw2wxyz(quat_xyzw), arrow_scale)
