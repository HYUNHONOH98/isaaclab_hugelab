...
def eetrack_subgoal_index_maxxed(env: ManagerBasedRLEnv) -> th.Tensor:
    ...
    reached_the_end = eetrack_command_term.current_eetrack_sg_index == (eetrack_command_term.number_of_subgoals)
    return reached_the_end


def eetrack_subgoal_index_maxxed_after_time(env: ManagerBasedRLEnv, time_after_maxxed: float) -> th.Tensor:
    eetrack_command_term = env.command_manager._terms["hands_pose"]
    dt_after_maxxed = int(time_after_maxxed // eetrack_command_term.env.step_dt)
    reached_the_end = eetrack_command_term.current_stepdt_after_first_sg_achieved == (
        eetrack_command_term.number_of_subgoals + dt_after_maxxed
    )
    return reached_the_end


def eetrack_pelvis_height_below_minimum(
...
def eetrack_bad_pelvis_ori(
    ...
    # torso_idx = asset.find_bodies(torso_body_name)[0][0]
    euler = math_utils.wrap_to_pi(
        th.stack(math_utils.euler_xyz_from_quat(asset.data.root_quat_w), dim=-1)
        # th.stack(
        #    math_utils.euler_xyz_from_quat(asset.data.root_quat_w), dim=-1)
        # math_utils.euler_xyz_from_quat(asset.data.body_state_w[:, torso_idx, 3:7]),
        # dim=-1,
...
def sit_done(
    ...
    """
    # extract the used quantities (to enable type-hinting)
    command: mdp.HeightCommand = env.command_manager.get_term(command_name)

    asset: RigidObject = env.scene[asset_cfg.name]
    is_done = th.abs(asset.data.root_pos_w[:, 2] - command.command[:, -1]) < height_threshold
    is_stable = th.norm(asset.data.root_lin_vel_w, dim=1) < lin_vel_threshold

    return th.logical_and(is_done.float(), is_stable.float())
