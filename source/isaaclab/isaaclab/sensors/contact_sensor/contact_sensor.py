...
class ContactSensor(SensorBase):
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        if self.cfg.track_air_time:
            ...
            self._data.last_contact_pose_w[env_ids] = 0.0
    ...
    def _initialize_impl(self):
        ...
        if self.cfg.track_pose:
            ...
            self._data.pose_w = torch.zeros(self._num_envs, self._num_bodies, 7, device=self._device)
            self._data.last_contact_pose_w = torch.zeros(self._num_envs, self._num_bodies, 7, device=self._device)
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        if self.cfg.track_pose:
            ...
            self._data.pose_w[env_ids] = pose
        ...
        if self.cfg.track_air_time:
            ...
            if self.cfg.track_pose:
                self._data.last_contact_pose_w[env_ids] = torch.where(
                    is_first_detached.unsqueeze(-1),
                    self._data.pose_w[env_ids],
                    self._data.last_contact_pose_w[env_ids],
                )
