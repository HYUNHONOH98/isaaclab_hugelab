...
class JointPositionAction(JointAction):
    ...
    def __init__(self, cfg: actions_cfg.JointPositionActionCfg, env: ManagerBasedEnv):
        ...
        self._torque_limits = (
            self._asset.root_physx_view.get_dof_max_forces().clone().to(self.device)[..., self._joint_ids]
        )
    ...
    def apply_actions(self):
        ...
        # Domain DR by applying random torque to each joint(https://proceedings.mlr.press/v242/campanaro24a/campanaro24a.pdf)
        if self.cfg.randomize_torque_rfi:
            # Generate random offsets in range [-1, 1]
            random_offsets = torch.rand_like(self.processed_actions) * 2.0 - 1.0
            # Scale by rfi_lim, rfi_lim_scale, and the actual torque limits
            torque_rfi = random_offsets * self.cfg.rfi_lim_scale * self._torque_limits
            # Finally, apply these torques to the articulation
